class Solution {
    public List<String> letterCombinations(String digits) {
        ...
        List<StringBuilder> responseBuilder = new LinkedList<>();
        for(String s : digitCharMap.get(digits.charAt(0))) {
            responseBuilder.add(new StringBuilder(s));
        }
        
        for(int i=1; i < len; ++i) {
            List<StringBuilder> temp = new LinkedList<>();
            for(String s : digitCharMap.get(digits.charAt(i))) {
                for(StringBuilder sb : responseBuilder) {
                    temp.add(new StringBuilder(sb.toString() + s));
                }
            }
            responseBuilder = temp;
        }

        return responseBuilder.stream().map(StringBuilder::toString).toList();
    }
