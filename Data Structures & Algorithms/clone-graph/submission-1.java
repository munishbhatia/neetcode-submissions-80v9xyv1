...
class Solution {
    ...
    private Node graphCloneHelper(Node node, Map<Integer, Node> valCloneNodeMap) {
        ...
        valCloneNodeMap.putIfAbsent(node.val, new Node(node.val));
        final Node clone = valCloneNodeMap.get(node.val);

        for(Node n : node.neighbors) {
            if(!valCloneNodeMap.containsKey(n.val)) {
                graphCloneHelper(n, valCloneNodeMap);
            }

            clone.neighbors.add(valCloneNodeMap.get(n.val));
        }
        return clone;
    }
}